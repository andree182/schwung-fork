...
- Notes below all split points are kept on the track (emitted further), adjusted by the global `transpose` parameter.
...
| Parameter | What it does |
|---------|--------|
| `split_oct_3` | Boundary octave for Split 3 (`off` or `C-1` to `C8`). |
| `split_3_chan` | MIDI channel for Split 3 (absolute `1` to `16`). |
| `pipe_3_select` | Named pipe to send Split 3 data (`1` to `8`, or `off` to route to current track). |
