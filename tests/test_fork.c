...
static int test_slot_recv_channel(void *instance) {
    (void)instance;
    return 3; // Mock Slot Receive Channel = 4 (index 3)
}
...
int main(void) {
    ...
    assert(fork_inst->main_oct_trans == 0);
    assert(fork_inst->split_1_oct_trans == 0);
    assert(fork_inst->split_2_oct_trans == 0);
    assert(fork_inst->split1_chan.is_relative == 1);
    assert(fork_inst->split1_chan.val == 4);
    assert(fork_inst->split2_chan.is_relative == 1);
    assert(fork_inst->split2_chan.val == 5);
    assert(fork_inst->recv_chan == -1); // Default is "track"
    assert(strcmp(fork_inst->recv_chan_str, "track") == 0);
    ...
    api->set_param(inst, "main_oct_trans", "-2");
    assert(fork_inst->main_oct_trans == -2);
    api->set_param(inst, "split_1_oct_trans", "1");
    assert(fork_inst->split_1_oct_trans == 1);

    api->set_param(inst, "split_1_chan", "10"); // Absolute channel 10 -> index 9
    assert(fork_inst->split1_chan.is_relative == 0);
    assert(fork_inst->split1_chan.val == 9);

    api->set_param(inst, "split_2_chan", "+4"); // Relative +4
    assert(fork_inst->split2_chan.is_relative == 1);
    assert(fork_inst->split2_chan.val == 4);

    api->set_param(inst, "recv_chan", "as_is");
    assert(fork_inst->recv_chan == -2);
    api->set_param(inst, "recv_chan", "5"); // Absolute Channel 5 -> index 4
    assert(fork_inst->recv_chan == 4);
    api->set_param(inst, "recv_chan", "track");
    assert(fork_inst->recv_chan == -1);

    // 3. Test Routing in Splitter Mode
    // split_oct_1 = C3 (48), split_oct_2 = C4 (60)
    // split_1_chan = absolute 10 (index 9)
    // split_2_chan = relative +4
    // main_oct_trans = -2 (transposes down 24 semitones)
    // split_1_oct_trans = 1 (transposes up 12 semitones)
    // split_2_oct_trans = 0
    ...
    assert(pop_ok == 1);
    assert(pop_len == 3);
    assert(pop_msg[0] == 0x95); // Channel 1 + 4 = 5 (index 5)
    assert(pop_msg[1] == 64);   // 64 + 0 = 64
    assert(pop_msg[2] == 90);
    ...
    pop_ok = ring_buffer_pop(&fork_inst->split1_ring_buf, pop_msg, &pop_len);
    assert(pop_ok == 1);
    assert(pop_msg[0] == 0xB9); // Channel adjusted to 9

    pop_ok = ring_buffer_pop(&fork_inst->split2_ring_buf, pop_msg, &pop_len);
    assert(pop_ok == 1);
    assert(pop_msg[0] == 0xB4); // Channel adjusted to 4 (relative +4)

    // 6. Test Receiver Mode & Channel Remapping
    api->set_param(inst, "mode", "receiver");
    assert(fork_inst->mode == 1);

    // A. Remap: "track" (maps to test_slot_recv_channel which returns index 3 / Channel 4)
    api->set_param(inst, "recv_chan", "track");
    uint8_t piped_msg[3] = { 0x91, 60, 100 }; // Note On on Channel 2
    ring_buffer_push(&fork_inst->recv_ring_buf, piped_msg, 3);
    count = api->tick(inst, 128, 44100, out_msgs, out_lens, 16);
    assert(count == 1);
    assert(out_msgs[0][0] == 0x93); // Remapped to Channel 4 (index 3)
    ...
    // C. Remap: explicit absolute channel index 5 (Channel 6)
    api->set_param(inst, "recv_chan", "6");
    ring_buffer_push(&fork_inst->recv_ring_buf, piped_msg, 3);
    count = api->tick(inst, 128, 44100, out_msgs, out_lens, 16);
    assert(count == 1);
    assert(out_msgs[0][0] == 0x95); // Remapped to Channel 6 (index 5)
    assert(out_msgs[0][1] == 60);
